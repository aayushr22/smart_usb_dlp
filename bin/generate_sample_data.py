...
def generate_session_duration(bytes_written):
    ...
    # Assuming average transfer rate of 50 MB per second for USB 3.0
    base_duration = bytes_written / (50 * 1048576)
    # randomness (20-200% of base duration)
...
def should_generate_anomaly(user, hour, day_of_week):
    ...
    anomaly_rates = {
        'normal': 0.02,      # 2% anomaly rate
        'light': 0.01,       # 1% anomaly rate
        'heavy': 0.03,       # 3% anomaly rate
        'suspicious': 0.15   # 15% anomaly rate
    }
    
    base_rate = anomaly_rates.get(profile, 0.02)
    
    # Increase anomaly probability for off-hours
    if hour in OFF_HOURS:
        base_rate *= 3
    
    # Increase anomaly probability for weekends
    if day_of_week in [0, 6]:  # Sunday = 0, Saturday = 6
        base_rate *= 2
...
def generate_usb_activity_data():
    ...
    data = []
    device_registry = {}  # Track devices per user for consistency
    
    # Generate data for each day
    for day_offset in range(DAYS_TO_GENERATE):
        current_date = datetime.datetime.now() - datetime.timedelta(days=day_offset)
        
        # Generate 50-200 events per day (realistic volume)
        ...
        for _ in range(daily_events):
            ...
            if random.random() < 0.7:  # 70% during working hours
                hour = random.choice(WORKING_HOURS)
            else:  # 30% during off hours
            ...
            # Select existing device (70% of time)
            ...
            # Generate anomaly type if needed
            anomaly_type = 'normal'
            if is_anomaly:
