...
def create_user_activity_chart():
    ...
    try:
        if db:
            ...
            if user_stats and user_stats.get('top_users'):
...
def create_device_risk_chart():
    ...
    try:
        if db:
            ...
            if device_stats and device_stats.get('devices_by_risk'):
...
# If USBDatabase/USBMLModel are None (import failed), assign mocks
if USBDatabase is None:
    USBDatabase = MockDatabase
if USBMLModel is None:
    USBMLModel = MockMLModel
