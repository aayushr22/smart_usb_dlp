        ...
        for i in range(num_samples):
            ...
            if is_anomaly:
                ...
                if anomaly_type == 'large_transfer':
                    bytes_written = np.random.randint(1000000000, 2000000000) 
                    hour = np.random.randint(8, 18)
                    day_of_week = np.random.randint(0, 5)
                elif anomaly_type == 'off_hours':
                    bytes_written = np.random.randint(1e6, 500e6) 
                    hour = np.random.choice([1, 2, 3, 22, 23])
                    day_of_week = np.random.randint(0, 5)
                elif anomaly_type == 'weekend':
                    bytes_written = np.random.randint(100e6, 1e9)
                    hour = np.random.randint(0, 24)
                    day_of_week = np.random.choice([5, 6])
                else:  # unknown_device
                    bytes_written = np.random.randint(1e6, 100e6)
                    hour = np.random.randint(8, 18)
                    day_of_week = np.random.randint(0, 5)
                    device_type = 'Unknown'
                ...
                bytes_written = np.random.lognormal(15, 1.2) 
